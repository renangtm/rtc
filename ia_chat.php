    ...
    <body>
        ...
        <div class="dashboard-main-wrapper">
                        ...
                        <div class="row">
                            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                <div class="page-header">
                                    <h2 class="pageheader-title">Inteligencia Chat</h2>
                                    <p class="pageheader-text">Nulla euismod urna eros, sit amet scelerisque torton lectus vel mauris facilisis faucibus at enim quis massa lobortis rutrum.</p>
                                    <div class="page-breadcrumb">
                                        <nav aria-label="breadcrumb">
                                            <ol class="breadcrumb">      
                                                <li class="breadcrumb-item"><a href="#" class="breadcrumb-link">RTC</a></li>
                                                <li class="breadcrumb-item active" aria-current="page">Inteligencia Chat</li>
                                            </ol>
                                        </nav>
                                    </div>
                                </div>
            ...
            ?>;
                                                ...
                                                function makeNode(xx, yy, nodo,pos) {
                                                    ...
                                                    mens[lm] = [];
                                                    crt[lm] = false;
                                                    var dvNome = $('<div></div>').html(pos+" Cod:"+nodo.id + " - " + (nodo.tipo === 1 ? "Fala" : (nodo.tipo === 0 ? "Resposta" : "<i class='fas fa-random'>&nbspRetorno")) + "<hr>" + nodo.expressao + (nodo.tipo>2?"Relacionado com "+(nodo.tipo-2):""));
                                                    ...
                                                    mens[lm][2] = dvRm;
                                                    var dvTipo = $('<button></button>').html('<i class="fa fa-certificate"></i>').addClass("btn btn-outline-warning");
                                                    mens[lm][3] = dvUpd;
                                                    var len = objetos.length;
                                                    objetos[objetos.length] = $('<div></div>').addClass('nodo_' + (nodo.tipo === 0 ? 'resposta' : (nodo.tipo === 1 ? 'fala' : 'retorno'))).css('position', 'absolute').css('width', w + 'px').css('height', h + 'px').css('left', (xx + x) + "px").css('top', (yy + y) + "px").appendTo(corpo).css('z-index', 1).append(dvNome).append($("<br></br>")).append(dvAdd).append(dvUpd).append(dvRm).append(dvTipo);
                                                    max--;
                                                    posicoes[posicoes.length] = {x: xx + x, y: yy + y, width: w, height: h};
                                                    dvAdd.click(function (e) {
                                                        
                                                        nodo.filhos[nodo.filhos.length] = {id:0,expressao:"",pai:null,filhos:[],tipo:0};

                                                        corpo.find('*').each(function () {
                                                            $(this).remove();
                                                        })
                                                        formarArvore(arvore, offset, 0,"1");
                                                        one = true;
                                                        gerarFundo();

                                                    })

                                                    dvTipo.click(function () {

                                                        nodo.tipo = (Math.min(2, nodo.tipo) + 1) % 3;

                                                        if (nodo.tipo === 2) {

                                                            var tmp = window.prompt("Digite o codigo do elemento que deseja relacionar");

                                                            if (tmp === null) {
                                                                return;
                                                            }

                                                            nodo.tipo = 2 + parseInt(tmp);

                                                        }

                                                        dvNome.html(nodo.id + " - " + (nodo.tipo === 1 ? "Fala" : (nodo.tipo === 0 ? "Resposta" : "<i class='fas fa-random'>&nbspRetorno")) + "<hr>" + nodo.expressao);
                                                        objetos[len].removeClass().addClass('nodo_' + (nodo.tipo === 0 ? 'resposta' : (nodo.tipo === 1 ? 'fala' : 'retorno')));
                                                    ...
                                                    dvUpd.click(function () {
                                                        ...
                                                        var tmp = window.prompt("Expressao");
                                                        if (tmp === null)
                                                            return;
                                                        nodo.expressao = tmp;
                                                        dvNome.html(nodo.id + " - " + (nodo.tipo === 1 ? "Fala" : (nodo.tipo === 0 ? "Resposta" : "<i class='fas fa-random'>&nbspRetorno")) + "<hr>" + nodo.expressao);
                                                ...
                                                function formarArvore(node, ofst, level,pos) {

                                                    if (node.filhos.length == 0) {

                                                        makeNode(level * (espacamento_horizontal + w), ofst, node,pos);
                                                        return h;
                                                    }
                                                    ...
                                                    for (var i = 0; i < node.filhos.length; i++) {
                                                        ...
                                                        var hr = formarArvore(node.filhos[i], nofst, level + 1,pos+"."+(i+1));
                                                        l[l.length] = hr;
                                                    ...
                                                    makeNode(level * (espacamento_horizontal + w), k, node,pos);
                                                    return nofst - ofst;
                                                }
                                                ...
                                                function salvar() {
                                                    jsBaseService({
                                                        o: {arvore: arvore},
                                                        query: "$org=new IAChat($empresa);$org->alterar($c,$o->arvore)",
                                                        sucesso: sucesso,
                                                        falha: sucesso
                                                    });
                                                }
