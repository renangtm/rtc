                                    ...
                                    <table class="table table-striped" style="margin-bottom: 10px;">
                                        ...
                                        <tbody>
                                            <tr ng-repeat="produto in pedido.produtos">
                                                ...
                                                <td class="text-center" style="color:{{(retirouPromocao(produto) > 0) ? 'Orange' : '#000000'}}">R$ {{produto.valor_base.toFixed(2)}} <span ng-if="retirouPromocao(produto) > 0"><hr>Seria R$ {{retirouPromocao(produto)}} em campanha, no entanto a campanha nao contempla este prazo</span></td>
                                                <td class="text-center" style="color:SteelBlue">R$ {{produto.base_calculo.toFixed(2)}}</td>
                                                <td class="text-center" style="{{produto.icms>0?'font-size:15px;color:DarkRed;text-decoration:underline':'text-decoration:line-through'}}">R$ {{produto.icms.toFixed(2)}}</td>
                                                <th class="text-center" style="{{produto.juros>0?'font-size:15px;color:DarkRed;text-decoration:underline':'text-decoration:line-through'}}">R$ {{produto.juros.toFixed(2)}}</th>
                                                <th class="text-center" style="{{produto.frete>0?'font-size:15px;color:DarkRed;text-decoration:underline':'text-decoration:line-through'}}">R$ {{produto.frete.toFixed(2)}}</th>
                                                <td class="text-center" style="color:Green">R$ {{((produto.valor_base + produto.icms + produto.frete + produto.ipi + produto.juros) * produto.quantidade).toFixed(2)}}</td>
                                            </tr>
