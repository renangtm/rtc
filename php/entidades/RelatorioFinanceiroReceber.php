...
class RelatorioFinanceiroReceber extends Relatorio {
    ...
    public function __construct($empresa = null) {
        ...
        parent::__construct("SELECT l.cliente as 'cliente',l.valor as 'valor',l.vencimento as 'vencimento',l.ficha as 'ficha',l.data as 'data',l.mes as 'mes',l.ano as 'ano',l.dia as 'dia',l.numero_nota as 'numero_nota' FROM (SELECT (CASE WHEN nota.saida THEN CONCAT(CONCAT(nota.id_cliente,' - '),cliente.razao_social) ELSE '---------' END) as 'cliente',(CASE WHEN nota.saida THEN '----------' ELSE CONCAT(CONCAT(nota.id_fornecedor,' - '),fornecedor.nome) END) as 'fornecedor',(GREATEST(ROUND((vencimento.valor-(SUM(IFNULL(movimento.valor,0)))),2),0)) as 'valor',UNIX_TIMESTAMP(vencimento.data)*1000 as 'vencimento', nota.ficha as 'ficha', vencimento.data as 'data', MONTH(vencimento.data) as 'mes', YEAR(vencimento.data) as 'ano', DAY(vencimento.data) as 'dia', nota.numero as 'numero_nota' FROM nota INNER JOIN vencimento ON vencimento.id_nota = nota.id LEFT JOIN cliente ON cliente.id=nota.id_cliente LEFT JOIN fornecedor ON fornecedor.id=nota.id_fornecedor LEFT JOIN movimento ON movimento.id_vencimento=vencimento.id WHERE nota.id_empresa=$empresa->id AND nota.saida=true AND nota.cancelada=false AND nota.excluida=false AND (CASE WHEN nota.saida THEN cliente.cnpj NOT IN $f ELSE fornecedor.cnpj NOT IN $f END) GROUP BY vencimento.id) l WHERE l.valor>2", 20);
